...
// ImportMovieFromTMDB creates a movie in our database from TMDB data
func (s *TMDBService) ImportMovieFromTMDB(tmdbID int, userID uint64) (CreateMovieInput, error) {
	// Get movie details
	movie, err := s.GetMovieDetails(tmdbID)
	if err != nil {
		return CreateMovieInput{}, err
	}

	// Parse release year
	var releaseYear int
	if movie.ReleaseDate != "" {
		if _, err := fmt.Sscanf(movie.ReleaseDate, "%d", &releaseYear); err != nil {
			releaseYear = 0
		}
	}
	...
	// Build input for our CreateMovie service
	input := CreateMovieInput{
		Title:          movie.Title,
		ReleaseYear:    releaseYear,
		Genres:         genres,
		Summary:        movie.Overview,
		PosterURL:      s.GetImageURL(movie.PosterPath, "w500"),
		BackdropURL:    s.GetImageURL(movie.BackdropPath, "original"),
